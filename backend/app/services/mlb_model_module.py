...
class MLBModelModule:
    """
    Standalone Python class to project the "Full Game" (9 Innings) scores for both Away and Home teams,
    along with Moneyline Probabilities, Totals, and Spread Advantages based on specific MLB Excel formulas.
    """

    def __init__(self, team_name_mapping, full_rpg_database, team_database, pitcher_metrics, bullpen_database):
        self.team_name_mapping = team_name_mapping
        self.full_rpg_database = full_rpg_database
    ...
    def _fetch_offense_stats(self, team_code):
        ...
        # Fetch RPG (Full Game)
        rpg_info = self.full_rpg_database.get(team_code, {})
        if isinstance(rpg_info, dict):
            # Try specific keys from full_rpg_database structure, fallback to 4.50
            rpg0 = rpg_info.get("current_2026", rpg_info.get("rpg0", 4.50))
        ...
        if isinstance(team_info, dict):
            wrc = float(team_info.get("wrc_plus", 100.0))
    ...
    def _fetch_pitching_stats(self, pitcher_name, team_code):
        ...
        if isinstance(sp_info, dict):
            sp0 = float(sp_info.get("xera", sp_info.get("siera", 4.20)))
    ...
    def calculate_score(self, offense_team, pitching_team, pitcher, is_home):
        ...
        rpg = max(3.0, min(6.0, rpg0)) if rpg0 > 0 else 4.50
        sp = max(2.0, min(8.0, sp0))
        bp = max(2.0, min(8.0, bp0))
        ...
        # 4. Finalizing
        final_score = round(max(0.0, min(15.0, score)), 1)
        return final_score
